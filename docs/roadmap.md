...
### The active thread: a live, fully server-driven client — **slice 1 done; slice 2 (WebSocket) next**
...
- **[ADR 0032](adr/0032-live-session-websocket.md) — the live session over a bidirectional WebSocket. Accepted; not started.** Request/response is superseded by one persistent, authenticated socket: the client streams *intents* (navigate, keystroke input, invoke), the server pushes *UI updates* into named regions (replace/append/patch), plus toasts and shell mutations — the region-update protocol [ADR 0029](adr/0029-sdui-emit-side.md) deferred. It buys **search-as-you-type** (debounced server-side so the addons aren't hammered) and **server push** (alerts, live changes); latency was explicitly *not* the justification. Costs: the server becomes stateful, owns reconnection/ordering/backpressure, and the protocol must be versioned. **This is the next slice.**

**Handoff for slice 2** (a fresh session will likely pick this up): the app shell is the natural first thing to push over the socket, so slice 2 changes the *channel* under a settled chrome model — a WebSocket transport on the Platform, a per-connection session holder, intent→command routing (reusing the application services unchanged), the region-update message vocabulary, and the Shell swapping its `fetchScreen` calls for socket intents + a region-update applier. The `SearchBar`'s `$value` binding and the search box actually driving search land naturally here (the client streams the field value as an `input` intent rather than needing client-side `$value` substitution).

**Carried gaps / cleanup:** `mosaic-sdui-react@0.1.2` (and `0.1.1`) are committed and running via a local build in the Shell's `node_modules`, but **not published to npm** — a fresh `npm install` reverts until published and the Shell's dep is bumped; the Shell's now-unused `mock/screens.ts` + `gallery/` are dead code to remove; and the boot-time loading race (searching during sign-in) is still open.
