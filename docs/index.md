...
- **The public SDK surface** is published, proven and extracted ([ADR 0016](adr/0016-published-contract-surface.md)). It is now its own module — [`github.com/mosaic-media/mosaic-sdk`](https://github.com/mosaic-media/mosaic-sdk) at `v0.1.0` — carrying the content services, models and an opaque `Caller`, with the store contracts kept internal. The Platform depends on it as an external module, and the reference capability uses only it to import an anime end to end against a real database. The critical path is complete.
