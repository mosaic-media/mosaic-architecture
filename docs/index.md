...
- **The public SDK surface** is published, proven and extracted ([ADR 0016](adr/0016-published-contract-surface.md)). It is its own module — [`github.com/mosaic-media/mosaic-sdk`](https://github.com/mosaic-media/mosaic-sdk) — carrying the content services, models and an opaque `Caller`, with the store contracts kept internal. `v0.1.0` extracted the content surface; `v0.2.0` adds the `Capability` interface a module implements ([ADR 0019](adr/0019-module-capability-and-invocation.md)). The Platform depends on it as an external module; the reference capability imported an anime end to end against a real database, and the first optional module (the Stremio addon source) is now composed in and invoked through the capability registry. The critical path is complete, and the composition-and-invocation half of the extension story with it.
